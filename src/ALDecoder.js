...
let UTIL_REGS = [
  {
    name: "PC",
    value: 1
  }, { // Holds the address in RAM of the next instruction to be executed
    name: "PSW",
    value: 0
...
class ALDecoder extends React.Component {
  constructor() {
    super();
    this.state = {
  ...
  render() {
    return (<div className="papaBear">
      <Grid>
        <Row>
          <Col md={8} sm={8} lg={8}>
            <CodeDisplay ref="codeDisp" width={this.state.codeDisplayWidth + "px"} timer={this.state.timer} timerChange={this.setTimer.bind(this)} compileCode={this.compileCode.bind(this)} testRun={this.playCode.bind(this)} changeCode={this.handleChange.bind(this)} changeNoti={this.setNotification.bind(this)} playCode={this.playCode.bind(this)} stopCode={this.setPause.bind(this)} stepCode={this.step.bind(this)} code={this.state.code} fullReset={this.fullReset.bind(this)} notification={this.state.notification} notiColor={this.state.notificationColor} selectedLine={UTIL_REGS[0].value}/>
          </Col>
          <Col md={4} sm={4} lg={4}>
            <DataDisplay opRegs={this.state.operationRegs} utilRegs={this.state.utilRegs} dataString={this.state.dataValStr} clearDataArray={this.memoryWipe.bind(this)} memoryOps={this.state.memory} updateInputMem={this.updateInputMem.bind(this)} updateDataArray={this.updateMemory.bind(this)}
              //Method that gets passed into the updateDataArray so the compiler knows what is input.
            />
          </Col>
        </Row>
      </Grid>
  ...
  executeLineOfCode() {
    ...
    if (loc >= instructions.length) {
      ...
      console.log(MEMORY);
      this.setState({utilRegs: UTIL_REGS, memoryOps: MEMORY_OPS, operationRegs: OP_REGS});
    }
  ...
  compressCode() {
    ...
    this.setState({code: seperatedLines})
  }
  compileCode() {
    this.reset();
    ...
    for (var i = 0; i < compiledCode.length; i++) {
      ...
      if (opType === InstrTypes.NONE) {
        success = false;
        this.setNotification("Operation in line " + (
        i + 1) + " is not found");
        setTimeout((function() {
          home.setNotification("")
        }), 3000);
        ...
        if (!e.includes(",")) {
          success = false;
          this.setNotification("Forgot to include comma in instruction " + (
          i + 1));
          setTimeout((function() {
            home.setNotification("")
          }), 3000);
        ...
        if (!regExists/* The Value Register does not exist */) {
          ...
          if (rd === "Zero") {
            ...
            this.setNotification("Dest Register in instruction " + (
            i + 1) + " does not exist");
          }
          success = false;
          setTimeout((function() {
        ...
        if (!regExists/* The Source Register does not exist */) {
          this.setNotification("Source Register in instruction " + (
          i + 1) + " does not exist");
          success = false;
          setTimeout((function() {
            home.setNotification("")
